...
const SimulationChamber = () => {
  ...
  const { sessionId, questions, jobTitle, interviewLengthMinutes } =
  ...
  useEffect(() => {
    ...
    elapsedSyncRef.current = setInterval(() => {
      if (sessionId) {
        interviewService.updateElapsedTime(sessionId, seconds).catch(() => {}); // silent fail — non-critical
      }
    }, 30000);
  ...
  return (
    <div className="w-full h-[100dvh] bg-abyss flex flex-col overflow-hidden relative select-none">
      <AbortPopup
        isOpen={isModalOpen}
      ...
      >
        ...
        <main className="w-full max-w-7xl mx-auto px-margin-edge mt-18 h-[calc(100vh-4.5rem)] flex flex-col justify-between py-6 z-10 relative">
          ...
          {isWarning && (
            <div
              className={`mb-4 border px-4 py-2 font-mono text-xs uppercase tracking-wider text-center transition-all duration-300
                            ${
                              isExpiring
          ...
          {quotaBlocked && (
            <div className="mb-4 border border-danger/40 bg-danger/10 px-4 py-2 font-mono text-xs text-danger uppercase tracking-wider text-center flex items-center justify-center gap-2">
              <ShieldOff size={12} />
              AI evaluation unavailable — daily quota exhausted. You may
              navigate questions but cannot submit answers.
            </div>
          )}

          {/* Question card */}
          <div className="bg-panel border-l-4 border-neon-blue p-4 md:p-6 shadow-2xl relative max-h-[40vh] md:max-h-[35vh] flex flex-col flex-shrink-0">
            <p className="text-steel font-metadata text-[10px] tracking-[0.3em] mb-2 uppercase font-bold flex-shrink-0">
              SEQUENCE {String(currentIndex + 1).padStart(2, "0")} /{" "}
            ...
            <div
              className={`flex-1 min-h-0 ${expanded || questionLength <= 300 ? "overflow-y-auto brutalist-scroll pr-2" : "overflow-hidden"}`}
            >
              <h1
                className={`text-pure-white ${questionSize} font-black leading-relaxed tracking-tight uppercase`}
              >
                {displayQuestion}
          ...
          {/* Footer controls */}
          <div className="flex flex-wrap items-center justify-between gap-3 pt-4 md:pt-6 pb-2 flex-shrink-0 w-full">
            {/* Left controls */}
            ...
            {/* Right controls */}
            <div className="flex items-center gap-2 md:gap-4 order-1 md:order-2 w-full md:w-auto">
              ...
              {!isCurrentSubmitted &&
                ...
                !quotaBlocked && (
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-3 md:px-6 py-3 md:py-3.5 border border-neon-blue text-neon-blue hover:bg-neon-blue/10 font-metadata text-[10px] md:text-xs uppercase tracking-widest transition-all duration-300 cursor-pointer disabled:opacity-50 w-full md:w-auto"
                  >
                    {isSubmitting ? (
              ...
              {isLastQuestion && isCurrentSubmitted && !quotaBlocked && (
                <button
                  ...
                  className={`relative flex items-center justify-center w-full md:w-auto px-2 md:px-6 py-3 md:py-3.5 border overflow-hidden transition-all duration-300 group border-neon-blue text-neon-blue hover:shadow-[0_0_15px_rgba(0,136,255,0.35)] cursor-pointer disabled:opacity-50`}
                  style={{
                    clipPath:
                      "polygon(6% 0, 100% 0, 100% 70%, 94% 100%, 0 100%, 0 30%)",
                  }}
                >
                  <span className="relative z-10 font-metadata text-[9px] md:text-xs uppercase tracking-[0.1em] md:tracking-[0.2em] font-black group-hover:text-abyss transition-colors duration-300 whitespace-nowrap">
                    {isCompleting ? (
                      <RefreshCw
                        size={12}
              ...
              {!isLastQuestion && isCurrentSubmitted && (
                <button
                  onClick={stepForward}
                  className="flex items-center justify-center gap-2 px-4 md:px-6 py-3 md:py-3.5 border border-neon-blue text-neon-blue hover:bg-neon-blue/10 font-metadata text-[10px] md:text-xs uppercase tracking-widest transition-all duration-300 cursor-pointer w-full md:w-auto"
                >
