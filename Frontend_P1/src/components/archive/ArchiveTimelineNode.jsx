...
const ICON_MAP = {
  ...
  stripe: (
    <CreditCard size={160} className="translate-x-1/4 -translate-y-1/4" />
  ),
  ...
  return (
    <div className="relative w-full group select-none [perspective:1200px]">
      ...
      >
        ...
        <div
          className={`absolute top-0 left-0 h-full w-1 transition-colors duration-500 ${isCompleted ? "bg-neon-blue/20 group-hover:bg-neon-blue" : "bg-frame group-hover:bg-warning"}`}
        />
        ...
        <div className="flex flex-col items-start md:items-end justify-center shrink-0 relative z-10 mt-6 md:mt-0 select-none">
          ...
          <div className="flex items-center gap-3">
            <span
              className={`font-mono text-5xl md:text-7xl font-black tracking-tighter leading-none ${isCompleted ? "text-neon-blue" : "text-warning"}`}
            >
              {isCompleted ? log.score : "--"}
              <span className="text-xl md:text-2xl font-mono text-steel/40 ml-0.5 font-bold">
                %
              </span>
            </span>
            {isCompleted ? (
              <CheckCircle2
                size={24}
                className="text-neon-blue drop-shadow-[0_0_10px_rgba(0,136,255,0.4)]"
              />
            ) : (
        ...
        <div
          className={`absolute right-0 top-0 opacity-5 group-hover:opacity-15 transition-all duration-700 pointer-events-none select-none text-neon-blue ${isCompleted ? "text-neon-blue" : "text-warning"}`}
        >
          {bgIcon}
        </div>
