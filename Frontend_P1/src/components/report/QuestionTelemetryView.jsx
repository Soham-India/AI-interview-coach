...
const QuestionTelemetryView = ({ data, isLast }) => {
  ...
  return (
    <div className="py-4 relative w-full h-full flex flex-col justify-center">
      ...
      <div
        className={`absolute -left-12 top-1/2 -translate-y-1/2 w-[22px] h-[22px] bg-abyss border-2 ${nodeBorder} rounded-full z-10 flex items-center justify-center select-none`}
      >
        <div
          className={`w-2 h-2 ${pulseBg} rounded-full ${isStrong ? "animate-pulse" : ""}`}
        />
      </div>
      ...
      <div className="pl-12">
        ...
        <div className="border border-frame bg-panel/60 overflow-hidden flex flex-col justify-between flex-shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 border-b border-frame/40">
            <div>
              <span
                className={`font-mono text-[10px] uppercase tracking-[0.25em] block mb-1 ${accentColor}`}
              >
            ...
            <div className="text-left sm:text-right mt-4 sm:mt-0 select-none">
              <span
                className={`font-mono text-5xl md:text-6xl font-black leading-none tracking-tighter ${accentColor}`}
              >
          ...
          <div className="p-6 bg-abyss/40 grid grid-cols-1 lg:grid-cols-3 gap-4 text-xs md:text-sm">
            ...
            <div className="space-y-4 border-r border-frame/20 pr-4">
              ...
              <div>
                <h6
                  className={`font-mono text-[10px] uppercase tracking-[0.25em] mb-1.5 font-bold select-none ${accentColor}`}
                >
                  YOUR RESPONSE FEED
                </h6>
